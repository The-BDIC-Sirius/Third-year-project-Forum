...
//         数据库中添加一个用户，该步暂时不会刷新缓存
        out.println(userEntity.getBlogsById());
    ...
    public String login(@RequestBody Map<String, String> map){
        if (map.containsKey("username")&&map.containsKey("userpwd")) {
            ...
            for (int i=0;i<userList.size();i++){
                if (username.equals(userList.get(i).getNickname())&&
                        password.equals(userList.get(i).getPassword())){
                    String jsonMessage = "{\"success\":\"true\",\"loginname\":\""+ username+"\"," +
                            "\"avatar_url\":\"https://avatars3.githubusercontent.com/u/25190477?v=4&s=120\",\"id\":\""+ userList.get(i).getId()+"\"}";
